...
def newUser(username, email, password):
  ...
  #conn.commit()
  cursor.execute(
    f"INSERT INTO userProfile (userName, email, password) VALUES ('{username}', '{email}', '{password}')"
  )
  conn.commit()
...
def checkUser(username, email):
  cursor.execute(
    f"SELECT * FROM userProfile WHERE username = '{username}' OR email = '{email}'"
  )
...
def updateUser(key, value, username):
  if key == "email":
    ...
    if check:
      ...
      cursor.execute(
        f"UPDATE userProfile SET email = '{value}' WHERE username = '{username}'"
      )
      conn.commit()
      return True
  ...
  if key == "password":
    cursor.execute(
      f"UPDATE userProfile SET password='{value}' WHERE username='{username}'")
    conn.commit()
...
def addHighscore(points, username):
  cursor.execute(
    f"UPDATE userProfile SET highscore = highscore+ {points} WHERE username = '{username}'"
  )
  conn.commit()
...
def checkHighscore(username):
  cursor.execute(
    f"SELECT highscore FROM userProfile WHERE username = '{username}'")
